...
function TubeGeometry({ scrollProgress, isFullscreen, onJourneyComplete }: TubeGeometryProps) {
  ...
  useFrame((state) => {
    if (tubeRef.current && lightRef.current) {
      ...
      if (isFullscreen) {
        // In fullscreen mode, directly control the main camera
        camera.position.copy(p1);
        camera.lookAt(p2);
        lightRef.current.position.copy(p2);
      } else if (cameraGroupRef.current) {
        // In non-fullscreen mode, use the camera group
  ...
  return (
    <group>
      {/* Camera Group for non-fullscreen mode only */}
      {!isFullscreen && (
        <group ref={cameraGroupRef} position={[0, 0, 400]}>
